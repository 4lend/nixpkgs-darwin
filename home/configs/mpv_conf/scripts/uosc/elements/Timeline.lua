...
function Timeline:render()
	...
	if (config.opacity.chapters > 0 and foreground_size > self.line_width and (#state.chapters > 0 or state.ab_loop_a or state.ab_loop_b)) then
		...
		if diamond_radius > 0 then
			local function draw_chapter(time, radius)
				...
				ass:draw_start()
				ass:move_to(chapter_x - radius, chapter_y + diamond_border)
				-- ass:line_to(chapter_x, chapter_y - radius)
				ass:line_to(chapter_x + radius, chapter_y + diamond_border)
				ass:line_to(chapter_x, chapter_y + radius + diamond_border)
				ass:draw_stop()
			end
